...
with st.sidebar:
    ...
    type_sel = st.multiselect("Types", ["loiter", "rendezvous", "holding", "convoy"], default=["loiter", "rendezvous", "holding", "convoy"])
...
with tabs[0]:
    ...
    layers = []
    for df, name in [(ais,"AIS"), (adsb,"ADSB"), (ground,"GROUND")]:
        if not df.empty:
            layers.append(pdk.Layer("ScatterplotLayer", data=df, get_position='[lon, lat]', get_radius=80, pickable=True))
    deck = pdk.Deck(map_style=None, initial_view_state=pdk.ViewState(latitude=34, longitude=-119, zoom=8), layers=layers)
