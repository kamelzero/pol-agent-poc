...
def simulate_vehicle(vid: str, lat: float, lon: float, mps: float, heading: float):
    while True:
        ...
        dlon = (mps / (meters_per_deg * math.cos(math.radians(max(min(lat,89.9),-89.9))))) * math.sin(math.radians(heading))
        lat += dlat
        lon += dlon
...
def main():
    ...
    while True:
        for g in gens:
            ...
            msg = Envelope(
                scenario_id=SCENARIO_ID, source="ground", ts=ts_now(),
                entity_id=vid, lat=lat, lon=lon, speed=spd, course=hdg,
                attrs={"type":"car"},
