...
def maritime_agent():
    ...
    while True:
        ...
        if len(series) > 60:
            ...
            if max(speeds) <= LOITER_SPEED_KTS and dur_min >= 10:
                anomaly = DomainAnomaly(
                    ts=series[-1].ts, domain="maritime", entity_id=env.entity_id, h3=env.h3,
                    type="loiter", score=0.7,
        ...
        recent = {eid:list(q) for eid,q in buf.items() if q and (now_utc() - datetime.fromisoformat(q[-1].ts)).total_seconds() < WIN_SEC}
            ...
            for j in range(i+1, len(last_points)):
                ...
                if (li.speed or 0) <= LOITER_SPEED_KTS and (lj.speed or 0) <= LOITER_SPEED_KTS:
                    ...
                    if d <= RENDEZ_RADIUS_M:
                        anomaly = DomainAnomaly(
                            ts=li.ts, domain="maritime", entity_id=f"{ei}|{ej}",
                            h3=li.h3, type="rendezvous", score=0.8,
